(function (factory) {
  ...
  function processTree_old(sourceFile, replacer) { //LM
    var code = '';
    ...
    function readThrough(node) {
      // LM
      var chunk = sourceFile.text.slice(cursorPosition, node.pos);
      code += '\r\n/******* ' + node.kind + ':' + sourceFile.text.slice(node.pos, node.end) + '************/\r\n'
      //if (chunk=='import') chunk = '// ' + chunk; 
      code += chunk;
      //  LM code += sourceFile.text.slice(cursorPosition, node.pos);
      cursorPosition = node.pos;
    }
    function visit(node) {
      //LM 231 TypeAliasDeclaration, 208 VariableStatement, 230 InterfaceDeclaration, 229 ClassDeclaration, 233 ModuleDeclaration

      if (node.kind == ts.SyntaxKind.ImportDeclaration || node.kind == ts.SyntaxKind.ExportAssignment) {
        //if (node.kind == 238 || node.kind == 243 ) {
        var chunk = sourceFile.text.slice(cursorPosition, node.pos);
        code += chunk;
        cursorPosition = node.end;
        return;
  ...
  function generate(options) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      output.write('declare namespace Mui {' + eol); //LM
      if (options.externs) {
